...
struct AlignMerge : public RcppParallel::Worker {
  ...
  const std::vector<std::string> s1v, s2v, q1v, q2v;

  // output merged sequence and quality scores are in an array
  RcppParallel::RVector<int> msvp;
  RcppParallel::RVector<int> mqvp;
  
  // Other variables
  int sqlen;

  
  // Initialize AlignMerge
  AlignMerge(const std::vector<std::string> s1v, const std::vector<std::string> s2v, 
             const std::vector<std::string> q1v, const std::vector<std::string> q2v,
             Rcpp::IntegerVector msv, Rcpp::IntegerVector mqv) : 
    s1v(s1v), s2v(s2v), q1v(q1v), q2v(q2v), msvp(msv), mqvp(mqv) {
  }
  ...
  void operator()(std::size_t begin, std::size_t end) {
    ...
    for(std::size_t j=begin;j<end;j++) {
      // Convert ACGT sequences into numbers
      char *seq1 = (char *) malloc(s1v[j].size()+1);
      char *seq2 = (char *) malloc(s2v[j].size()+1);
      if (seq1 == NULL || seq2 == NULL)  Rcpp::stop("Memory allocation failed.");
      ::nt2int(seq1, s1v[j].c_str());
      ::nt2int(seq2, s2v[j].c_str());
      sqlen = s1v[j].size();
      
      // Run endsfree alignment
      al = ::himap_nwalign_endsfree(seq1, seq2, q1v[j].c_str(), q2v[j].c_str(), 
                                    c_score, GAP_P);
      // Merge aligned reads
      merged = ::himap_merge_alignment(al);
      msvp[j] = int(*merged[0]);
...
// [[Rcpp::export]]
Rcpp::List C_parallel_mergepairs(
    std::vector<std::string> s1_v,
  ...
  // Output variables
  Rcpp::IntegerVector ms_v, mq_v;
  Rcpp::CharacterVector ms, mq;
  ...
  // Add alignment parameters as arguments later
  AlignMerge align_merge(s1_v, s2_v, q1_v, q2_v, ms_v, mq_v);
  
  // Run alignment and merging in parallel
  RcppParallel::parallelFor(0, s1_v.size(), align_merge);
  
  // Post-process results which are integer vectors back into characters
  // Rcpp::IntegerVector::iterator its, itq;
  // for (its = ms_v.begin(), itq = mq_v.begin(); its < ms_v.end(); ++its, ++itq) {
  //   *its
  // }
  // 
  return(Rcpp::List::create(ms_v, mq_v));
}
