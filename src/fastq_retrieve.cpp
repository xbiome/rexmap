...
// [[Rcpp::export]]
map<int, vector<int> > value_map (vector<int> x) {
...
// [[Rcpp::export]]
map<int, vector<int> > partid_to_fastqid (vector<int> dada_map, vector<int> derep_map) {
  // Generate map partitionID -> fastqID
  // inputs are maps from paritionID -> uniqueIDs and uniqueID -> fastqIDs
  map<int, vector<int> > pid_to_uid = value_map(dada_map);
  map<int, vector<int> > uid_to_fid = value_map(derep_map);
  // Now combine these into paritionID -> fastqIDs
  map<int, vector<int> > pid_to_fid;
  for (map<int, vector<int> >::const_iterator ptu_it = pid_to_uid.begin(); ptu_it != pid_to_uid.end(); ++ptu_it) {
    pid_to_fid[ptu_it->first] = {};
    for (vector<int>::const_iterator u_it = ptu_it->second.begin(); u_it != ptu_it->second.end(); ++u_it) {
...
// [[Rcpp::export]]
string consensus_sequence (vector<string> seqs) {
  string cons;
  ...
  int max_id;
  for (j=0; j<len; ++j) {
    max_id = distance(cons_mat+j*6, max_element(cons_mat+j*6, cons_mat+j*6+6));
...
/*** R
# x =  c(1, 2, 3, 4, 1, 2, 5, 1, 6, 4, 7, 8, 8)-1
pid_to_uid = c(0, 0, 0, 1)
uid_to_fid = c(0, 0, 0, 1, 2, 2, 3, 4, 3, 4)
seqs = c('CCAGTAGTCC', 'CTAGTAGTC', 'CGGGTAGTC', 'CTGGTAGTC', 'CTGGTAGTC')
*/
